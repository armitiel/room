...
random.seed(12)
OUT=Path(r"C:\Users\DELL\Room\work\scenes\attic-room-v01")
OUT.mkdir(parents=True,exist_ok=True)
bpy.ops.object.select_all(action="SELECT"); bpy.ops.object.delete(use_global=False)
for block in bpy.data.materials: bpy.data.materials.remove(block)
W,L,H,K,R=3.2,4.5,2.55,1.08,1.55
...
wx0,wx1,wz0,wz1=2.0,2.92,.95,2.14
profile=[(0,0),(W,0),(W,H),(R,H),(0,K)]
front=mesh("Entrance end wall",[(x,-.06,z) for x,z in profile],[(0,1,2,3,4)],wall)
...
sx0,sx1,sy0,sy1=.32,1.14,1.55,2.42
roofs=[]
...
for x in [wx0,wx1]:box("Gable window jamb",(x,L-.045,(wz0+wz1)/2),(.065,.12,wz1-wz0+.08),wood,.006)
for z in [wz0,wz1]:box("Gable window rail",((wx0+wx1)/2,L-.045,z),(wx1-wx0+.06,.12,.065),wood,.006)
...
area("Soft window daylight",(2.4,4.24,1.9),(1.2,1.2,.6),180,1.1,(.84,.91,1))
area("Skylight daylight",(.85,2.0,2.08),(2.6,2,.3),140,.8,(.90,.94,1))
area("Interior fill",(2.1,1.0,2.33),(1.8,3,.5),95,2.2,(1,.90,.81))
...
cam1=camera("01 View from entrance",(2.80,.36,1.65),(1.64,3.18,1.16),20)
cam2=camera("02 View towards door",(1.75,4.12,1.58),(2.14,.53,1.26),20)
...
scene["dimensions_status"]="All dimensions estimated; awaiting user measurements."
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/"Room-attic-v01.blend"))
bpy.ops.export_scene.gltf(filepath=str(OUT/"Room-attic-v01.glb"),export_format="GLB",export_cameras=False,export_lights=False)
...
(OUT/"dimensions.json").write_text(json.dumps({"status":"estimated_not_measured","width_m":W,"length_m":L,"max_height_m":H,"knee_wall_height_m":K,"slope_run_m":R,"bed_m":[2.05,1.66],"source":"18 unique photos IMG_0416-IMG_0433 and 22 sampled frames of IMG_0415.MOV","note":"Manual visual reconstruction; no camera solving or photogrammetry performed."},indent=2))
